...
import static PokerCard.ACE;
import static PokerCard.JACK;
...
/**
    ...
    public boolean isNatural(List <PokerCard> handOfPokerCards){
        ...
        if(handOfPokerCards.get(0).getRankCode()==ACE&&handOfPokerCards.get(1).getRankCode()>=JACK&&handOfPokerCards.get(2).getRankCode()>=JACK) return true;
        if(handOfPokerCards.get(1).getRankCode()==ACE&&handOfPokerCards.get(0).getRankCode()>=JACK&&handOfPokerCards.get(2).getRankCode()>=JACK) return true;
        if(handOfPokerCards.get(2).getRankCode()==ACE&&handOfPokerCards.get(0).getRankCode()>=JACK&&handOfPokerCards.get(1).getRankCode()>=JACK) return true;
        return false;
    }
